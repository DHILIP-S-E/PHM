...
export default function Dashboard() {
    ...
    const fetchGlobalData = async () => {
        try {
            setLoading(true);
            const [wRes, sRes, uRes, mRes] = await Promise.all([
                warehousesApi.list({ size: 1 }),
                shopsApi.list({ size: 1 }),
                employeesApi.list({ size: 1 }), // Assuming employeesApi covers users for now
                medicinesApi.list({ size: 1 })
            ]);

            setStats({
                warehouses: wRes.data?.total || 0,
                shops: sRes.data?.total || 0,
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto">
            ...
            {/* Entity-Specific Sections: Charts, Alerts, Quick Actions - HIDDEN IN GLOBAL SCOPE */}
            {scope !== 'global' && (
                <>
                    {/* Alert Cards */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
